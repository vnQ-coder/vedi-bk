...
class BaseDao<Model extends BaseModel, TblCols extends string|number|symbol = keyof Model> {
  ...
  getAll<Model>(trx: Transaction, fields: TblCols[]|string = "*"): QueryBuilder<Model[], Model[]> {
    return trx(this.tableName).select(fields);
  }

  findAllPickField<Model>(trx: Transaction, fields: TblCols): QueryBuilder<Model[], Model[]> {
  ...
  findOneByCol(
    ...
    return trx(this.tableName).where(colName as string, value).first(fields);
  }
  ...
  insertMany(trx: Transaction, models: Model[] | Model | Partial<Model>): QueryBuilder<Model[], Model[]> {
    return trx(this.tableName).insert(models).returning(this.returnedCols);
  }

  insertOne(trx: Transaction, model: Model|Partial<Model>): QueryBuilder<Model[], Model[]> {
  ...
  upsertManyByCol(
    ...
    return this.upsertMany(trx, models, conflictConstraint)
      .where(`${this.tableName}.${colName}`, colValueByUpdate);
  }
